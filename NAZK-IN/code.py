...
root = "/home/ubuntu/sanctions-scripts/NAZK-IN/"
# root = ""
...
def sourcedownloader():
    url = "https://sanctions.nazk.gov.ua/en/download-person-warning/"
    r = requests.get(url)
    try:
...
def process_data():
    ...
    frame_ = df.to_dict("records")

    # copy data to list
    for rec in frame_:
        item = {}
        name_ = rec['Name En'].split(" / ")[0]
    
        item["uid"] = get_hash(name_, str(rec['Country']))
        item["name"] = name_

        item["alias_name"] = [str(rec['Name En']).strip(),
                            str(rec['Name Ru']).strip(),
                            str(rec['Name Uk']).strip()]
        ali = alias_name(name_)
        if ali:
            item['alias_name'].append(ali)
        item['country'] = []
        item["list_type"] = "Individual"
        item["last_updated"] = last_updated_string
        item['individual_details'] = {
            "date_of_birth" : [rec['date of birth'].strip("dm.").strip("mm.").replace(".","/").strip()],
            "place_of_birth" : [rec['birthplace En'].strip("\r\n").strip()]
        }
        item['designation'] = [rec['Position En']]
        item['sanction_details'] = {}
        item['nns_status'] = "False"
        item['address'] = {}
        item['documents'] = {}
        item['comment'] = rec['Reasoning En']
        item['sanction_list'] = {
            "sl_authority": "National Agency on Corruption Prevention, Ministry of Foreign Affairs, Ukraine",
            "sl_url": "https://sanctions.nazk.gov.ua/en/",
            "sl_host_country":"Ukraine",
            "sl_type": "Sanctions",
            "watch_list": "European Watchlists",
            "sl_source": "Ministry of Foreign Affairs, National Agency on Corruption Prevention Potential Sanctions Subjects, Ukraine",
            "sl_description": "List of individuals and organizations who are potential sanctions subject by National Agency on Corruption Prevention, Ministry of Foreign Affairs, Ukraine",
            "list_id" : "UKR_E20299"
        }
