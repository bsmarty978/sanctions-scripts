...
dag_name = "bajaj-836"
root = "/home/ubuntu/sanctions-scripts/BAJAJ-836/"
...
def get_hash(n):
    return hashlib.sha256(((n+'Bajaj Allianz Life Insurance Co. Ltd. Black Listed Agents List, India (India Watchlists IND_E20345)').lower()).encode()).hexdigest()
...
def sourcedownloader():
    ...
    res =  requests.get("https://www.bajajallianzlife.com/content/dam/balic/pdf/agents-terminated-on-account-of-malpractices.pdf")
...
def process_data():
    ...
    tb = camelot.read_pdf(f'{ip_path}/{input_filename}', pages="all", flavor="stream",columns=["0","1"])
    print(len(tb))
    # exit()
    ...
    names = []
    for t in tb:
        t.df.fillna("",inplace=True)
        mdf = t.df[5:]
        for i,r in mdf.iterrows():
            ml  = r[1].split("\n")
            if len(ml)==4:
                name = " ".join(ml[0].split(" ")[1:])
                name = name.replace("Mr.","").replace("Ms.","").replace("M/s","").replace("Mrs.","").replace("mr.","").replace("mrs.","").replace(".","").strip()
                pan = ml[1]
                odate = ml[2]
            elif len(ml)==5:
                name = ml[1]
            ...
            d = {
                ...
                "comment": "", 
                "sanction_list": {
                    "sl_authority": "Bajaj Allianz Life Insurance Co. Ltd. Black Listed Agents List, India",
                    "sl_url": "https://www.bajajallianzlife.com/",
                    "sl_host_country": "India",
                    "sl_type": "Sanctions",
                    "sl_source": "Bajaj Allianz Life Insurance Co. Ltd. Black Listed Agents List, India",
                    "sl_description": "Bajaj Allianz Life Insurance Co. Ltd. Black Listed Agents List, India",
