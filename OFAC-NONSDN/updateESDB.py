...
def UpdateLocalJsonFile(diffdata,rmdata):
    nsdl_root = "/home/ubuntu/nsdl-files/"
    alpha_file = "alpha_data.json"
    try:
        with open(f'{nsdl_root}{alpha_file}','r') as file:
            all_data = json.load(file)
        

        #NOTE : Adds Latest data in to Local File
        all_data = all_data + diffdata
        with open(f'{nsdl_root}{alpha_file}', 'w', encoding="utf-8") as f:
            json.dump(all_data, f, ensure_ascii=False, indent=4) 

    except Exception as e:
        print("Error While Updating local JSON FIle")
        print(f"ERROR : {e}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Updating LOCAL JSON FILE")
        UpdateLocalJsonFile(diffdata=file_data,rmdata=remo_data)
        print("Completed Updating LOCAL JSON FILE")
