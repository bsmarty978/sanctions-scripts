...
dag_name = "sng-826"
root = "/home/ubuntu/sanctions-scripts/SNG-826/"
...
def get_hash(n):
    return hashlib.sha256(((n+"Bajaj Allianz General Insurance Co. Ltd. Black Listed Agents List, India (India Watchlists IND_E20335)").lower()).encode()).hexdigest()
...
def sourcedownloader():
    ...
    res =  requests.get("https://www.bajajallianz.com/download-documents/agent-details/agent-details.pdf",headers=header)
    # resp = HtmlResponse("example.com",body=r.text,encoding='utf-8')
    # url = "https://life.futuregenerali.in"+resp.xpath("//a[text()='Terminated Agents']/@href").get()
    # print(url)
    # exit()
...
def process_data():
    ...
    for i in range(1,809,50):
        ...
        tb = camelot.read_pdf(f'{ip_path}/{input_filename}', pages=f"{s}-{e}", flavor="lattice")
        print(len(tb))
        # print(tb[0].df.head())
        # print(tb[0].df.columns)
        # exit()
        for d in tb:
            for r,j in d.df.iterrows():
                data_dict = {}
                status = j[4].strip()
                if status not in ["Blacklisted", "License Cancelled"]:
                    continue
                
                name = j[1].strip()
                name = name.replace('\n', '').strip('.').strip()
                
                issue_date = j[3].strip()
                licence_no = j[2].strip()
                ...
                data_dict['sanction_details'] = {'issue_date':issue_date, 'licence_no':licence_no}
                data_dict['documents'] = {"PAN":[]}
                ...
                data_dict["sanction_list"]= {
                    ...
                    "sl_source": "Bajaj Allianz General Insurance Co. Ltd. Black Listed Agents List, India",
                    "sl_description": "List of blacklisted agents by Bajaj Allianz General Insurance Co. Ltd",
                    "list_id": "IND_E20335"
                }
