...
root = "/home/ubuntu/sanctions-scripts/NAZK-P/"
# root = ""
...
def get_hash(n, a):
    return hashlib.sha256(((n+"Ministry of Foreign Affairs, National Agency on Corruption Prevention Sanctions List, Ukraine UKR_S10049"+a).lower()).encode()).hexdigest()
...
def sourcedownloader():
    url = "https://sanctions.nazk.gov.ua/en/sanction-person/?csv=1"
    r = requests.get(url)
    try:
...
def process_data():
    ...
    for rec in frame_:
        ...
        dob = rec['date of birth'].strip("dm.").strip("mm.").replace(".","/").strip()
        if dob:
            dobs = [dob]
        else:
            dobs = []

        pob = rec['birthplace En'].strip("\r\n").strip()
        if pob:
            pobs = [pob]
        else:
            pobs = []

        if rec['Position En'].strip():
            desi = [rec['Position En'].strip()]
        else:
            desi = []
