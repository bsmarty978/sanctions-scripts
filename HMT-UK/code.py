...
def process_data():
    ...
    for index, row in graw.iterrows():
        ...
        regime = [ri for ri in row["Regime"] if ri][0]
        # listedon = [li for li in row["Listed On"] if li][0]


        pass_dict["uid"] = hashlib.sha256(((str(gid) + "HM Tressuary, UK" + "UK_S10008").lower()).encode()).hexdigest()
        pass_dict["name"] = name
        ...
        if ltype == "Individual":
            ...
            pass_dict["individual_details"]["place_of_birth"] = pob_adrs
            pass_dict["designation"] = desig
        else:
            pass_dict["entity_details"] = {}
        pass_dict["documents"] = {}
        pass_dict["documents"]["passport"] = passports
        pass_dict["documents"]["id"] = nids
        pass_dict["comment"] = regime
