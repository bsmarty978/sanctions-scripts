...
class conlist():
    ...
    def uploadToS3(self):
        try:
            ...
            s3 = boto3.client('s3')
            s3.upload_file(f'{self.inputpath}/{self.date}_input_uk.csv', "sams-scrapping-data", f"HMT-UK/Original/{self.date}_input_uk.csv")
            s3.upload_file(f'{self.outputpath}/{self.date}_output_uk.json', "sams-scrapping-data", f"HMT-UK/Parced/{self.date}_output_uk.json")
            s3.upload_file(f'{self.Differencepath}/{self.date}_diffrance_uk.json', "sams-scrapping-data",f"HMT-UK/Diffrance/{self.date}_diffrance_uk.json")
            s3.upload_file(f'{self.Removedpath}/{self.date}_removed_uk.json', "sams-scrapping-data",f"HMT-UK/Removed/{self.date}_removed_uk.json")
            s3.upload_file(self.lp_path, "sams-scrapping-data",f"HMT-UK/{self.lp_path}")
    ...
    def parsing(self):
        ...
        try:
            try:
                # f =  open(f'{self.inputpath}/{self.date}_DifferenceFile_uk.csv',newline='')
                # f = open(f'{self.inputpath}/{self.date}_input_uk.csv', newline='',encoding='windows-1252')
                f = open(f'{self.inputpath}/{self.date}_input_uk.csv', newline='')
            except:
                    ...
                    try:
                        ...
                        if item['groupid'] in temp:
                            ...
                            if list_type == "Individual":
                                ...
                                if i[10]:
                                    item['country'].append(i[10])
                                item['documents'] = {}
                                item['documents']['passport'] = i[11]
                                ...
                                item['sanction_list']['sl_url'] = "https://www.gov.uk/government/publications/financial-sanctions-consolidated-list-of-targets/consolidated-list-of-targets"
            ...
            try:
                for ii in list:
                    ...
                    ii.pop("groupid")
                # ab = json.dumps(list)
                f = open(f"{self.outputpath}/{self.date}_output_uk.json", "w",encoding='utf-8')
                # f.write(ab)
                global out_obj
