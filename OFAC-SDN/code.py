...
base_individual_obj = {
        ...
        "sanction_list": {
            ...
            "sl_type": "Specially Designated Nationals",
...
input_filename  = f'sdn-xml-{today_date.day}-{today_date.month}-{today_date.year}.xml'
output_filename = f'sdn-json-{today_date.day}-{today_date.month}-{today_date.year}.json'
diffrance_filename = f'diffrance-sdn-json-{today_date.day}-{today_date.month}-{today_date.year}.json'
removed_filename = f'removed-sdn-json-{today_date.day}-{today_date.month}-{today_date.year}.json'
old_output_filename = f'sdn-json-{yesterday.day}-{yesterday.month}-{yesterday.year}.json'

#NOTE: Paths of directories
root = "/home/ubuntu/sanctions-scripts/OFAC-SDN/"
ip_path = f"{root}inputfiles"
op_path = f"{root}outputfiles"
dp_path = f"{root}diffrancefiles"
rm_path = f"{root}removedfiles"
lp_path = f"{root}ofac-sdn-logfile.csv"
...
def fileprocessor():
    ...
    for entry in sda_data_obj:
        ...
        copy_obj["sanction_list"] = {
            ...
            "sl_url": "https://home.treasury.gov/policy-issues/office-of-foreign-assets-control-sanctions-programs-and-information", #for SDN list
            "sl_host_country": "United States of America",
            "watch_list" : "Global Watchlists",
            # "sl_type": "NON-SDN(Sanctions)",
            "sl_type": "Specially Designated Nationals", #NOTE: For SDN lists
...
def UploadfilestTos3():
    try:
        ...
        s3 = boto3.client('s3')
        s3.upload_file(f'{ip_path}/{input_filename}',"sams-scrapping-data",f"OFAC/sdn/original/{input_filename}")
        s3.upload_file(f'{op_path}/{output_filename}',"sams-scrapping-data",f"OFAC/sdn/parced/{output_filename}")
        s3.upload_file(f'{dp_path}/{diffrance_filename}',"sams-scrapping-data",f"OFAC/sdn/diffrance/{diffrance_filename}")
        s3.upload_file(f'{rm_path}/{removed_filename}',"sams-scrapping-data",f"OFAC/sdn/removed/{removed_filename}")
        s3.upload_file(f'{lp_path}',"sams-scrapping-data","OFAC/sdn/ofac-sdn-logfile.csv")
