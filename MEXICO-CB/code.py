...
dag_name = "mexico-cb"
root = "/home/ubuntu/sanctions-scripts/MEXICO-CB/"
...
def get_hash(n):
    return hashlib.sha256(((n+"mexico centrel bank MEX_E20205").lower()).encode()).hexdigest()    
            

def process_data():
    global total_profile_available,out_list,trans_dict
    ...
    for y in range(2009,2022):
        ...
        for tr in resp.xpath("//table[@id='sanciones:sanciones']/tbody/tr"):
            pass_dict =  {
                ...
                "country": [],
                "list_type": "Entity",
                "last_updated": "2022-05-23T23:05:11",
                "entity_details": {},
                "nns_status": "False",
                "address": [
                {
            ...
            try:
               tname = trans_dict[name]
            except:
                tname = langtranslation(name)
                trans_dict[name] = tname

            try:
               tsboy = trans_dict[sbody]
            except:
                tsboy = langtranslation(sbody)
                trans_dict[sbody] = tsboy

            print(tname)
            pass_dict["uid"] = get_hash(name)
            pass_dict["name"] = tname.strip()
            pass_dict["sanction_details"]["body"] = tsboy.strip()
            pass_dict["sanction_details"]["penalty_amount"] = tax_amount.strip()

            out_list.append(pass_dict)

    for y in range(2009,2022):
        url = f"https://www.banxico.org.mx/ServiciosFinancieros/sanciones/publicaciones.xhtml?idDisposicion=3&idIntermediario=40&anio={y}&pecuniaria=false&tipoConsulta=I"
        res = requests.get(url)
        resp = HtmlResponse("example.com",body=res.text,encoding='utf-8')
        for tr in resp.xpath("//table[@id='sanciones:sanciones']/tbody/tr"):
            pass_dict =  {
                ...
                "comment": "",
                "sanction_list": {
                    "sl_authority": "Central Bank of Mexico, Mexico",
                    "sl_url": "https://www.banxico.org.mx/ServiciosFinancieros/sanciones/publicacionesTipos.xhtml?idDisposicion=3&idIntermediario=40&anio=2021&tipoConsulta=I",
                    "sl_host_country": "Mexico",
                    "sl_type": "Sanctions",
                    "watch_list": "South and Central America Watchlists ",
                    "sl_source": "Central Bank of Mexico Sanctions List, Mexico",
                    "sl_description": "List of sanctioned entities by Central Bank of Mexico Sanctions List, Mexico.",
                    "list_id": "MEX_E20205"
                }
                }
            name = tr.xpath("./td[1]/text()").get()
