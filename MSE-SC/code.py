...
dag_name = "mse-sc"
root = "/home/ubuntu/sanctions-scripts/MSE-SC/"

input_filename  = f'{dag_name}-inout.xlsx'
...
def get_hash(n):
    return hashlib.sha256(((n+'MSE Suspended Companies, India IND_E20325').lower()).encode()).hexdigest()
...
def sourcedownloader():
    ...
    r =  requests.get("https://www.msei.in/corporates/corporate-securities-information/Suspended-Companies")
    resp = HtmlResponse("example.com",body=r.text,encoding='utf-8')
    url = resp.xpath("//td[@class='txtcntr t-icon']/a/@href").get()
    ...
    for i,row in df.iterrows():
        d = {
            ...
            "sanction_list": {
                ...
                "sl_url": "https://www.msei.in/corporates/corporate-securities-information/Suspended-Companies",
                "watch_list": "India Watchlists",
                "sl_host_country": "India",
                "sl_type": "Sanctions",
                "sl_source": "MSE Suspended Companies, India",
                "sl_description": "List of the suspended companies by Metropolitan Stock Exchange, India",
                "list_id": "IND_E20325"
            }
        }
        ...
        symbol = row["Symbol"]
        isin = row["ISIN"]
        odate  = row["Suspension Date"]
        comment = row["Reason for suspension"]
        if not name:
            continue
        ...
        d["name"] = name
        if symbol:
            d["alias_name"].append(symbol)
        if isin:
            d["documents"]["ISIN"].append(isin)
        if odate:
