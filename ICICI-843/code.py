...
dag_name = "icici-843"
root = "/home/ubuntu/sanctions-scripts/ICICI-843/"
...
def get_hash(n, pan):
    return hashlib.sha256(((n+"ICICI Prudential Life Insurance Co. Ltd. Black Listed Agents List, India"+pan).lower()).encode()).hexdigest()

def pan_splitter(pan):
    newPan = pan.strip().split(' ')
    comment = ""
    
    if len(newPan)>1:
        pans = str(newPan[0]).strip()
        comment  = " ".join(newPan[1:])
...
def sourcedownloader():
    ...
    res =  requests.get("https://www.iciciprulife.com/content/dam/icicipru/download-centre/others/negative-agents/Negative_Agent_List.pdf")
...
def process_data():
    ...
    # exit()
    for i in tb:
        for r, j in i.df.iterrows():
            data_dict = {}
            name = j[0].strip()
            name = name.replace('\n', '').strip('.').strip()
            issue_date = j[1].strip()
            inpan = j[2].strip()
            incom = j[3].strip()
            if inpan:
                pan, comment = pan_splitter(inpan)
            
                if not comment:
                    comment = j[3].strip()
            else:
                pan, comment = pan_splitter(incom)
        
            
            if name == 'Name' or name == "BLACKLISTED AGENT'S LIST AS OF 31st May 2022":
                continue
            
            comment = comment.replace('\n', '').strip('.').strip()
            
            data_dict['uid'] = get_hash(name, pan)
            data_dict['name'] = name
            ...
            data_dict["sanction_list"]= {
                "sl_authority": "ICICI Prudential Life Insurance Co. Ltd., India",
                "sl_url": "https://www.iciciprulife.com/",
                "sl_host_country": "India",
                "sl_type": "Sanctions",
                "watch_list": "India Watchlists",
                "sl_source": "ICICI Prudential Life Insurance Co. Ltd. Black Listed Agents List, India",
                "sl_description": "List of black listed agents issued as per the guidelines of IRDAI by ICICI Prudential Life Insurance Co. Ltd., India",
                "list_id": "IND_E20352"
            }
