...
dag_name = "IndiaFirst-844"
root = "/home/ubuntu/sanctions-scripts/IndiaFirst-844/"
...
def get_hash(n, agentID):
    return hashlib.sha256(((n+"IndiaFirst Life Insurance Co. Ltd Black Listed Agents List, India (India Watchlists"+agentID).lower()).encode()).hexdigest()
...
def sourcedownloader():
    ...
    r =  requests.get("https://www.indiafirstlife.com/others-ph",headers=header)
    resp = HtmlResponse("example.com",body=r.text,encoding='utf-8')
    url = resp.xpath("//a[text()='Terminated Agents']/@href").get()
    # url = url.replace("\\","/")
    print(url)
    # exit()
...
def process_data():
    ...
    for i in tb:
        for r, j in i.df.iterrows():
            ...
            if name == 'Name':
                    continue
            agent_code = j[1].strip()
            issue_date = j[4].strip()
            comment = j[5].strip()
            
            data_dict['uid'] = get_hash(name, agent_code)
            data_dict['name'] = name
            ...
            data_dict["sanction_list"]= {
                "sl_authority": "IndiaFirst Life Insurance Co. Ltd, India",
                "sl_url": "https://www.indiafirstlife.com/others-ph",
                "sl_host_country": "India",
                "sl_type": "Sanctions",
                "watch_list": "India Watchlists",
                "sl_source": "IndiaFirst Life Insurance Co. Ltd Black Listed Agents List, India",
                "sl_description": "List of blacklisted agents by IndiaFirst Life Insurance Co. Ltd, India",
                "list_id": "IND_E20353"
            }
