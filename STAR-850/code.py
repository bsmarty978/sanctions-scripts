...
dag_name = "star-850"
root = "/home/ubuntu/sanctions-scripts/STAR-850/"
...
def get_hash(n):
    return hashlib.sha256(((n+"Star Union Dai-Ichi Life Insurance Co. Ltd. Black Listed Agents List, India").lower()).encode()).hexdigest()
...
def sourcedownloader():
    ...
    res =  requests.get("https://www.sudlife.in/Pdf%20Attachment/List_of_Terminated_Advisor.pdf",headers=header)
    ...
    for d in tb:
        for i,r in d.df.iterrows():
            ...
            if i == 0:
                continue
            name = r[2].strip()
            if name == 'Advisor Name' or name == "":
                continue
            ns = name.split(" ")
            name =  " ".join(ns[1:])
            code = ns[0]
            issue_date = r[0].strip()
            term_date = r[3].strip()
            comment = r[4].strip()
            ...
            data_dict["sanction_list"]= {
                "sl_authority": "Star Union Dai-Ichi Life Insurance Co. Ltd. Black Listed Agents List, India",
                "sl_url": "https://www.sudlife.in/Policies-Disclosures",
