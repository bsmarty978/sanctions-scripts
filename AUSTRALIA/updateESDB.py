...
output_filename = f'{today_date}_output_au.json'
diffrance_filename =f'{today_date}_diffrance_au.json'
removed_filename =f'{today_date}_removed_au.json'
...
def check_data_of_esdb(es,identity,article):
    ...
    global failed_uids
    try:
        res = es.index(index=db_name,id=identity, document=article)
        success_c += 1
    except Exception as ex:
        failure_c += 1
        print(f"Data Inserting Error For UID : {identity}")
