...
root = "/home/ubuntu/sanctions-scripts/HK-BnE/"
# root = ""
...
def process_data():
    ...
    for div in divs:
        ...
        wb = []
        for w in website:
            if "N/A"!=w and w.strip():
                wb.append(w.replace("[","").replace("]",""))

        name = name.replace(",","").replace(" "," ")
        # name = name.replace(",","")
        if name in list(out_dict.keys()):
            out_dict[name]['entity_details']['website'].extend(wb)
        else:
            item = {}
            item['uid'] = hashlib.sha256(((name + "Hong Kong Monetary Authority Fraudulent Banks and Entities List, Hong Kong" + "HKG_E20158").lower()).encode()).hexdigest()
            item['name'] = name
            item['alias_name'] = []
            item['country'] = []
            ...
            item['entity_details'] = {}
            item['entity_details']['website'] = wb
